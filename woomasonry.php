  ...
  <button class="button is-checked" data-filter="*">show all</button>';
	...
	if ( $wm_catTerms <> '' ) {
		...
		foreach ( $wm_catTerms as $woo_term ) {
			...
			$wm_prod_args = array(
				'post_type'      => 'product',
				'posts_per_page' => $wm_atts['number'],
				'product_cat'    => $woo_term->slug,
			);
			...
			' .
						apply_filters(
							...
							sprintf(
								'<a href="%s" rel="nofollow" data-product_id="%s" data-product_sku="%s" class="woomscart button %s product_type_%s">%s</a>',
